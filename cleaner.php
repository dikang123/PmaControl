...
class SqlFormatter
{
    ...
    protected static $reserved               = array(
        ...
        'DO', 'DUMPFILE', 'DUPLICATE', 'DYNAMIC', 'ELSE', 'ENCLOSED', 'END', 'ENGINE', 'ENGINE_TYPE', 'ENGINES', 'ESCAPE', 'ESCAPED', 'EVENTS',
        'EXECUTE',
        ...
        'ON UPDATE', 'ON DELETE', 'OUTFILE', 'PACK_KEYS', 'PAGE', 'PARTIAL', 'PARTITION', 'PARTITIONS', 'PASSWORD', 'PRIMARY', 'PRIVILEGES',
        'PROCEDURE',
    ...
    protected static $functions              = array(
        ...
        'DAYOFYEAR', 'DECODE', 'DEFAULT', 'DEGREES', 'DES_DECRYPT', 'DES_ENCRYPT', 'DIFFERENCE', 'DIMENSION', 'DISJOINT', 'DISTANCE', 'ELT',
        'ENCODE',
        'ENCRYPT', 'ENDPOINT', 'ENVELOPE', 'EQUALS', 'EXP', 'EXPORT_SET', 'EXTERIORRING', 'EXTRACT', 'EXTRACTVALUE', 'FIELD', 'FIND_IN_SET',
        'FLOOR',
        ...
        'GET_LOCK', 'GLENGTH', 'GREATEST', 'GROUP_CONCAT', 'GROUP_UNIQUE_USERS', 'HEX', 'HOUR', 'IF', 'IFNULL', 'INET_ATON', 'INET_NTOA', 'INSERT',
        'INSTR',
        'INTERIORRINGN', 'INTERSECTION', 'INTERSECTS', 'INTERVAL', 'ISCLOSED', 'ISEMPTY', 'ISNULL', 'ISRING', 'ISSIMPLE', 'IS_FREE_LOCK', 'IS_USED_LOCK',
        'LAST_DAY', 'LAST_INSERT_ID', 'LCASE', 'LEAST', 'LEFT', 'LENGTH', 'LINEFROMTEXT', 'LINEFROMWKB', 'LINESTRING', 'LINESTRINGFROMTEXT',
        'LINESTRINGFROMWKB',
        'LN', 'LOAD_FILE', 'LOCALTIME', 'LOCALTIMESTAMP', 'LOCATE', 'LOG', 'LOG10', 'LOG2', 'LOWER', 'LPAD', 'LTRIM', 'MAKEDATE', 'MAKETIME',
        'MAKE_SET',
        'MASTER_POS_WAIT', 'MAX', 'MBRCONTAINS', 'MBRDISJOINT', 'MBREQUAL', 'MBRINTERSECTS', 'MBROVERLAPS', 'MBRTOUCHES', 'MBRWITHIN', 'MD5',
        'MICROSECOND',
        'MID', 'MIN', 'MINUTE', 'MLINEFROMTEXT', 'MLINEFROMWKB', 'MOD', 'MONTH', 'MONTHNAME', 'MPOINTFROMTEXT', 'MPOINTFROMWKB', 'MPOLYFROMTEXT',
        'MPOLYFROMWKB',
        ...
        'OLD_PASSWORD', 'ORD', 'OVERLAPS', 'PASSWORD', 'PERIOD_ADD', 'PERIOD_DIFF', 'PI', 'POINT', 'POINTFROMTEXT', 'POINTFROMWKB', 'POINTN',
        'POINTONSURFACE',
        'POLYFROMTEXT', 'POLYFROMWKB', 'POLYGON', 'POLYGONFROMTEXT', 'POLYGONFROMWKB', 'POSITION', 'POW', 'POWER', 'QUARTER', 'QUOTE', 'RADIANS',
        'RAND',
        'RELATED', 'RELEASE_LOCK', 'REPEAT', 'REPLACE', 'REVERSE', 'RIGHT', 'ROUND', 'ROW_COUNT', 'RPAD', 'RTRIM', 'SCHEMA', 'SECOND', 'SEC_TO_TIME',
        'SESSION_USER', 'SHA', 'SHA1', 'SIGN', 'SIN', 'SLEEP', 'SOUNDEX', 'SPACE', 'SQRT', 'SRID', 'STARTPOINT', 'STD', 'STDDEV', 'STDDEV_POP',
        'STDDEV_SAMP',
        'STRCMP', 'STR_TO_DATE', 'SUBDATE', 'SUBSTR', 'SUBSTRING', 'SUBSTRING_INDEX', 'SUBTIME', 'SUM', 'SYMDIFFERENCE', 'SYSDATE', 'SYSTEM_USER',
        'TAN',
        'TIME', 'TIMEDIFF', 'TIMESTAMP', 'TIMESTAMPADD', 'TIMESTAMPDIFF', 'TIME_FORMAT', 'TIME_TO_SEC', 'TOUCHES', 'TO_DAYS', 'TRIM', 'TRUNCATE',
        'UCASE',
    ...
    public static function format($string, $highlight = true)
    {
        ...
        foreach ($tokens as $i => $token) {
            ...
            if ($token[self::TOKEN_VALUE] === '(') {
                ...
                for ($j = 1; $j <= 250; $j++) {
                    ...
                    if ($next[self::TOKEN_TYPE] === self::TOKEN_TYPE_RESERVED_TOPLEVEL || $next[self::TOKEN_TYPE] === self::TOKEN_TYPE_RESERVED_NEWLINE
                        || $next[self::TOKEN_TYPE] === self::TOKEN_TYPE_COMMENT || $next[self::TOKEN_TYPE] === self::TOKEN_TYPE_BLOCK_COMMENT) {
                        break;
                    }
            ...
            elseif ($clause_limit && $token[self::TOKEN_VALUE] !== "," && $token[self::TOKEN_TYPE] !== self::TOKEN_TYPE_NUMBER && $token[self::TOKEN_TYPE]
                !== self::TOKEN_TYPE_WHITESPACE) {
                $clause_limit = false;
            }
            ...
            if ($token[self::TOKEN_VALUE] === '-' && isset($tokens[$i + 1]) && $tokens[$i + 1][self::TOKEN_TYPE] === self::TOKEN_TYPE_NUMBER
                && isset($tokens[$i - 1])) {
                $prev = $tokens[$i - 1][self::TOKEN_TYPE];
                if ($prev !== self::TOKEN_TYPE_QUOTE && $prev !== self::TOKEN_TYPE_BACKTICK_QUOTE && $prev !== self::TOKEN_TYPE_WORD && $prev
    ...
    public static function splitQuery($string)
    {
        ...
        foreach ($tokens as $token) {
            ...
            if ($token[self::TOKEN_TYPE] !== self::TOKEN_TYPE_WHITESPACE && $token[self::TOKEN_TYPE] !== self::TOKEN_TYPE_COMMENT && $token[self::TOKEN_TYPE]
...
class PmaCliDraining
{
    ...
    public function start()
    {
        ...
        mysqli_select_db($this->db, $this->schema_to_purge);
    ...
    public function feedDeleteTableWithFk()
    {
        ...
        if ($this->debug) {
            echo "Liste des tables à effacer (niveau par niveau) : \n";
            print_r($list_tables);
        }
        ...
        foreach ($list_tables as $sub_array) {
            ...
            foreach ($sub_array as $table_name) {
                ...
                //$fks = mysqli_to_array($res);
                $fks = mysqli_fetch_all($res, MYSQLI_ASSOC);
    ...
    public function getTypeOfPrimaryKey($table)
    {
        ...
        if (mysqli_num_rows($res) === "0") { // should be == 1 have to fix it for PROD_LOT_ITEM
            throw new \Exception("GLI-067 : this table [".$table."] haven't primary key !");
        } else {
    ...
    public function log($sql)
    {
        if ($this->debug) {
            ...
            echo Color::getColoredString("-- row affected : ".mysqli_affected_rows($this->db), "blue", "black")."\n";
        }
    }
    ...
    private function getVirtualForeignKeys()
    {
        ...
        /*
          ...
          $foreign_keys = mysqli_fetch_yield($sql);
    ...
    private function getOrderBy($order = 'ASC')
    {
        ...
        while ($last = count($array) != 0) {
            ...
            if ($last == count($array)) {
                $cas_found = false;
                //cas de deux chemins differents pour arrivé à la même table fille (avec une difference de niveau)
                $temp      = $array;
    ...
    public function setAffectedRows($table)
    {
        ...
        if (empty($this->rows_to_delete[$table])) {
            $this->rows_to_delete[$table] = mysqli_affected_rows($this->db);
        } else {
            $this->rows_to_delete[$table] += mysqli_affected_rows($this->db);
        }
    }
    ...
    private function removeTableNotImpacted($fks)
    {
        ...
        echo "Les tables qui ne sont pas concernées : \n";
        ...
        do {
            ...
            foreach ($fks as $table => $data) {
                ...
                if (!$found) {
                    ...
                    if ($this->debug) {
                        echo "We removed this table (Not a child of : `".$this->schema_to_purge."`.`".$this->main_table."`) : ".$table.PHP_EOL;
                    }
...
$array = parse_ini_file("cleaner.ini", true);


$db = mysqli_connect($array['mysql']['ip'], $array['mysql']['login'], $array['mysql']['password'], $array['cleaner']['database'],
    $array['mysql']['port']);


$purge = new PmaCliDraining($db);

$purge->schema_to_purge = $array['cleaner']['database'];
$purge->schema_delete   = $array['cleaner']['database_for_clean'];
$purge->prefix          = $array['cleaner']['prefix'];
$purge->debug           = false;

if (!empty($_SERVER['argv'])) {
    foreach ($_SERVER['argv'] as $elem) {
        if ($elem == "--debug") {
            $purge->debug = true;
...
if ($purge->debug) {
    ...
    echo "Paramètres :\n";
    echo Color::setColor("grey", "green", "bold");
    print_r($array);
    echo Color::setColor("grey", "black", "normal");
    echo "\n";
}


$purge->main_table = $array['cleaner']['table'];

$i = 1;

$WAIT_TIME = $array['cleaner']['wait_time'];
//$this->getMsgStartDaemon($purge);


echo "[".date("Y-m-d H:i:s")."] Starting Cleaner\n";
$sql = "SELECT PK FROM `".$purge->schema_to_purge."`.`".$purge->main_table."` \nWHERE ".$array['cleaner']['query']."\n";


//mysqli_close

do {
    $purge->init_where = $array['cleaner']['query'];
    ...
    try {
        ...
        echo Color::getColoredString($ex->getMessage(), "grey", "red");
        echo "\n";
        mysqli_close($db);
        exit;
    }
    ...
    if ($ret[$purge->main_table] < 1) {
        break;
    }


    sleep($array['cleaner']['wait_time']);
    sleep(1);
} while ($ret[$purge->main_table] != "0");
